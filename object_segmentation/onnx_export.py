...
try:
    image = Image.open(requests.get(config["image_url"], stream=True).raw).convert(
        "RGB"
    )
...
class Sam3ONNXWrapper(torch.nn.Module):
    ...
    def forward(
        self, pixel_values, input_ids, attention_mask, input_boxes, input_boxes_labels
    ):
        outputs = self.sam3(
        ...
        return (
            ...
            outputs.decoder_reference_boxes,
        )
